...
router.put("/:username/permissions/add", auth, async (req, res) => {
  ...
  if (!util.checkPermission(req.user, "ADD_PERMISSIONS")) {
    return res.status(403).send({
      error:
        "Please contact an admin to request the needed permission to perform this operation",
  ...
  if (!permissions_added.length) {
    return res.status(400).send({ error: "Permissions to be added is empty!" });
  }
  ...
  return res.status(200).send({
    message: "Successfully added permissions to the user",
    payload: payload,
  });
});
...
router.delete("/:username/permissions/remove", auth, async (req, res) => {
  // Check if User can change permissions
  if (!util.checkPermission(req.user, "REMOVE_PERMISSIONS")) {
    return res.status(403).send({
      error:
        "Please contact an admin to request the needed permission to perform this operation",
  ...
  return res.status(200).send({
    message: "Successfully removed permissions from the user",
    payload: payload,
  });
});
...
router.post("/:username/permissions/reset", auth, async (req, res) => {
  // Check if User can change permissions
  if (!util.checkPermission(req.user, "RESET_PERMISSIONS")) {
    return res.status(403).send({
      error:
        "Please contact an admin to request the needed permission to perform this operation",
      permission: "RESET_PERMISSIONS",
    });
  }
  const user = await User.findOne({ username: req.params.username });
  // check if the user is available
  ...
  return res.status(200).send({
    message: "Successfully reset permissions for the user",
    payload: payload,
  });
});
...
router.get("/:username/permissions", auth, async (req, res) => {
  if (!req.params.username) {
    return res.status(400).send({ error: "Please send a user!" });
  }
  if (!util.checkPermission(req.user, "GET_PERMISSIONS")) {
    return res.status(403).send({
      error:
        "Please contact an admin to request the needed permission to perform this operation",
