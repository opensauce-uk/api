...
router.get("/", async (req, res) => {
  ...
  return res.status(200).send({
    message: "Returned list of all posts!",
    payload: array,
    action: "FETCH_POSTS",
  });
});
...
router.post("/new", auth, async (req, res) => {
  if (!util.checkPermission(req.user, "CREATE_POST")) {
    return res.status(401).send({
      error:
        "Please contact an admin to request the needed permissions to complete this operation",
...
router.delete("/delete", auth, async (req, res) => {
  ...
  if (!post) {
    return res.status(404).send({ error: "Could not find post to delete!" });
  }
  if (post.user == req.user.username) {
    post.deleteOne({ id: req.body.postid });
    res.status(200).send({ message: "Post has been deleted" });
  } else {
    res.status(401).send({ error: "Access Token provided is invalid" });
  }
});
